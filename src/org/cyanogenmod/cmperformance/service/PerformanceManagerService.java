...
public class PerformanceManagerService extends Service {
    ...
    static {
        ...
        if (DEBUG) Log.d(TAG, "Loaded jni library");
    }
    ...
    private static final int POWER_HINT_CPU_BOOST    = 0x00000010;
    private static final int POWER_HINT_LAUNCH_BOOST = 0x00000011;
    private static final int POWER_HINT_SET_PROFILE  = 0x00000030;
    ...
    public void onCreate() {
        ...
        if (activities != null && activities.length > 0) {
            ...
            for (int i = 0; i < activities.length; i++) {
                ...
                if (info.length == 2) {
                    ...
                    if (DEBUG) {
                        Log.d(TAG, String.format("App profile #%d: %s => %s",
                                i, info[0], info[1]));
                    }
                }
            }
        ...
        if (mNumProfiles > 0) {
            ...
            if (profile == PerformanceManager.PROFILE_HIGH_PERFORMANCE) {
                if (DEBUG) Log.i(TAG, String.format("Reverting profile %d to %d",
    ...
    private boolean getProfileHasAppProfilesInternal(int profile) {
        if (profile < 0 || profile > mNumProfiles) {
            Log.e(TAG, "Invalid profile: " + profile);
            return false;
        }
    ...
    private synchronized boolean setPowerProfileInternal(int profile, boolean fromUser) {
        ...
        if (mPm == null) {
            Log.e(TAG, "System is not ready, dropping profile request");
            return false;
        }
        if (profile < 0 || profile > mNumProfiles) {
            Log.e(TAG, "Invalid profile: " + profile);
    ...
    private void cpuBoostInternal(int duration) {
        synchronized (PerformanceManagerService.this) {
            if (mPm == null) {
                Log.e(TAG, "System is not ready, dropping cpu boost request");
                return;
            }
        }
    ...
    private final IBinder mBinder = new IPerformanceManagerProvider.Stub() {
        ...
        public void launchBoost(int pid, String packageName) throws RemoteException {
            synchronized (PerformanceManagerService.this) {
                if (mPm == null) {
                    Log.e(TAG, "System is not ready, dropping launch boost request");
                    return;
                }
            }
            // Don't send boosts if we're in another power profile
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_SET_PROFILE:
                    ...
                    Intent intent = new Intent(PerformanceManager.POWER_PROFILE_CHANGED);
                    intent.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
                    sendBroadcastAsUser(intent, UserHandle.ALL,
                            Manifest.permission.PERFORMANCE_ACCESS);
